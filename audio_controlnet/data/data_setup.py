...
from .extracted_audio import custom_collate_fn
from torch.utils.data.distributed import DistributedSampler
from typing import Sequence

from audio_controlnet.data.extracted_audio import ExtractedAudio
from audio_controlnet.data.mm_dataset import MultiModalDataset
...
def load_audio_data(cfg: DictConfig, data_cfg: DictConfig) -> Dataset: 
    dataset = ExtractedAudio(metadata_path=data_cfg.metadata, 
                            ...
                            background_metadata=data_cfg.get('background_metadata', None)
                            )
    return dataset
...
def setup_training_datasets(cfg: DictConfig) -> tuple[Dataset, DistributedSampler, DataLoader]:
    ...
    if cfg.mini_train: 
        ...
        # audiocaps_npz = load_audio_data(cfg, cfg.data.AudioCaps_npz) x
        ...
        if not isinstance(cfg.data.train_split, str):
            # multiple datasets
            datasets = []
            for sp in cfg.data.train_split:
                datasets.append(
                    load_audio_data(cfg, cfg.data[sp])
                )
            dataset = MultiModalDataset([], datasets) 
                
        else:
            dataset = load_audio_data(cfg, cfg.data[cfg.data.train_split])
...
def setup_test_datasets(cfg):  # used in sample
    ...
    dataset = load_audio_data(cfg, cfg.data[cfg.data.test_split])
...
def setup_val_datasets(cfg: DictConfig) -> tuple[Dataset, DataLoader, DataLoader]:
    ...
    dataset = load_audio_data(cfg, cfg.data[cfg.data.val_split])
...
def construct_loader(dataset: Dataset,
    ...
    train_loader = DataLoader(dataset,
                              ...
                              pin_memory=pin_memory,
                              collate_fn=error_avoidance_collate if error_avoidance else custom_collate_fn)
