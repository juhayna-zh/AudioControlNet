...
class FlowMatching:
    ...
    def __init__(self, min_sigma: float = 0.0, inference_mode='euler', num_steps: int = 25, from_data_to_noise = False):
        ...
        self.from_data_to_noise = from_data_to_noise
        # assert self.from_data_to_noise is True
    ...
    def loss(self, predicted_v: torch.Tensor, x0: torch.Tensor, x1: torch.Tensor, loss_coef=None) -> torch.Tensor:
        # return the mean error without reducing the batch dimension
        reduce_dim = list(range(1, len(predicted_v.shape)))
        if self.from_data_to_noise:
            target_v = (1 - self.min_sigma)* x0 - x1
        else:
            target_v = x1 - (1 - self.min_sigma) * x0
        if loss_coef is None:
            return (predicted_v - target_v).pow(2).mean(dim=reduce_dim)
        else:
            return ((predicted_v - target_v).pow(2) * loss_coef).mean(dim=reduce_dim)
