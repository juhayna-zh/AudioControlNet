...
from audio_controlnet.ext.rotary_embeddings import compute_rope_rotations
from audio_controlnet.model.embeddings import TimestepEmbedder
...
class PreprocessedConditions:
    ...
    control: Optional[Dict[str, torch.Tensor]] = field(default=None)
...
class FluxAudio(nn.Module):
    ...
    def __init__(self,
        ...
        self.joint_blocks = nn.ModuleList([
            JointBlock(hidden_dim,
                         ...
                         use_lora=use_lora,
                         lora_rank=lora_rank) for i in range(depth - fused_depth)  # last layer is pre-only (only appllied to text and vision)
        ])

        self.fused_blocks = nn.ModuleList([
            MMDitSingleBlock(hidden_dim, num_heads, mlp_ratio=mlp_ratio, kernel_size=3, padding=1, use_lora=use_lora, lora_rank=lora_rank)
    ...
    def initialize_rotations(self):
        ...
        self.calc_rot = lambda seq_len: compute_rope_rotations(seq_len, self.hidden_dim // self.num_heads, 10000, freq_scaling=base_freq, device=self.device)
    ...
    def predict_flow(self, latent: torch.Tensor, t: torch.Tensor,
                     conditions: PreprocessedConditions, return_latents = False, return_depth = 2**31) -> torch.Tensor:
        """
...
class ControlFluxAudio(FluxAudio):
    def __init__(self, 
                 base_model_checkpoint, 
                 control_types: List[str], 
                 conditioners_config: Dict[str, Dict], 
                 controlnets_config: Dict[str, Dict] = {}, 
                 use_lora: bool = False,
                 **kwargs
                 ):
        ...
        if use_lora:
            ...
            for name, param in self.named_parameters():
                if "lora" in name.lower():
                    lora_params_cnt += param.numel()
                else:
                    param.requires_grad_(False)
            log.info(f"Lora parameters number: {lora_params_cnt}")
        else:
        ...
        self.controlnets_config = controlnets_config
        
        self.empty_control_dim = {}
        for name in self.control_types:
            self.conditioners[name] = create_conditioner_from_config(name, conditioners_config[name])
    ...
    def _init_controlnet(self, control_name, **kwargs):
        ...
        if control_cfg.type == 'controlnet-origin':
            ...
            controlnet.latent_std.requires_grad_(False)
            controlnet.empty_string_feat.requires_grad_(False)
            ...
            controlnet = nn.ModuleDict()
            adapters = []
            for d in range(control_cfg.control_depth):
                adapters.append(CrossAttentionAdapter(**control_cfg.cross_attn))
            controlnet['adapters'] = nn.ModuleList(adapters)
            controlnet['extractor'] = Conv1dFeatureExtractor(**control_cfg.extractor)
            self.controlnet_models[control_name] = controlnet
            self.control_latent_proj[control_name] = nn.ModuleList([ZeroConv(control_cfg.cross_attn.dim, control_cfg.cross_attn.dim, channel_last=True) for i in range(self.depth)])
        else:
            raise NotImplementedError(f'Not implementation for controlnet type: {control_cfg.type}')
    
    def forward(self, latent: torch.Tensor, text_f: torch.Tensor, text_f_c: torch.Tensor, t: torch.Tensor, control: Optional[Dict[str, Any]] = None) -> torch.Tensor:
        """
        latent: (B, N, C) 
        text_f: (B, T, D)
        t: (B,)
        """
        conditions = self.preprocess_conditions(text_f, text_f_c, control)  # cachable operations 
        flow = self.predict_flow(latent, t, conditions)  # non-cachable operations
        return flow
    
    
    def preprocess_conditions(self, text_f: torch.Tensor, text_f_c: torch.Tensor, control: Optional[Dict[str, Any]] = None) -> PreprocessedConditions:  
        conditions = super().preprocess_conditions(text_f, text_f_c)
        if len(self.control_types) > 0 and control is None and not self._flag_is_getting_empty_condition:
            log.warning(f"The control_types is set to {self.control_types}, but control input is {control}. Please be careful, this may be an incorrect input!")
            traceback.print_stack()
        if control is not None:
            conditions.control = {
                name: self.conditioners[name](control[name], self.device) for name in self.control_types
            }
        return conditions
    
    def _forward_controlnet(self, t: torch.Tensor, conditions: PreprocessedConditions, latent: torch.Tensor) -> Optional[Dict[str, List[torch.Tensor]]]:
        if conditions.control is None:
            return None
        ...
        for name, cond_inp in conditions.control.items():
            if self.controlnets_config[name]['type'] == 'controlnet-origin':
                ...
                for i, lat in enumerate(layer_latents):
                    latents_dict[name].append(self.control_latent_proj[name][i](lat))
            elif self.controlnets_config[name]['type'] == 'cross-attn-adapter':
                latents_dict[name] = self.controlnet_models[name]['extractor'](cond_inp.permute(0, 2, 1)).permute(0, 2, 1)
            else:
                raise NotImplementedError(f"{name}: {self.controlnets_config[name]['type']}")
    ...
    def predict_flow(self, latent: torch.Tensor, t: torch.Tensor,
        ...
        is_use_controlnet = bool(conditions.control is not None)

        text_f = conditions.text_f
        text_f_c = conditions.text_f_c
        
        if is_use_controlnet:
            control_latents = self._forward_controlnet(t, conditions, latent)
        ...
        for i, block in enumerate(self.joint_blocks):
            ...
            if is_use_controlnet:
                for name in conditions.control:
                    if self.controlnets_config[name]['type'] != 'cross-attn-adapter':
                        continue
                    start_depth = self.controlnets_config[name].get('control_start_depth', 0)
                    if i >= start_depth and i - start_depth < self.controlnets_config[name].control_depth:
                        this_rot = None if control_latents[name].shape[1] == self._latent_seq_len else self.calc_rot(control_latents[name].shape[1])
                        latent_adapters.append(
                            lambda lat, name=name, idx=i-start_depth, rot=this_rot: 
                                self.control_latent_proj[name][idx](
                                    self.controlnet_models[name]["adapters"][idx](lat, control_latents[name], self.latent_rot, rot)
                                )
                        )
        ...
        for j, block in enumerate(self.fused_blocks):
            latent_adapters = []
            if is_use_controlnet:
                for name in conditions.control:
                    if self.controlnets_config[name]['type'] != 'cross-attn-adapter':
                        continue
                    start_depth = self.controlnets_config[name].get('control_start_depth', 0)
                    if self.mm_depth + j >= start_depth and self.mm_depth + j - start_depth < self.controlnets_config[name].control_depth:
                        this_rot = None if control_latents[name].shape[1] == self._latent_seq_len else self.calc_rot(control_latents[name].shape[1])
                        
                        latent_adapters.append(
                            lambda lat, name=name, idx=self.mm_depth + j - start_depth, rot=this_rot:
                                self.control_latent_proj[name][idx](
                                    self.controlnet_models[name]["adapters"][idx](lat, control_latents[name], self.latent_rot, rot)
                                )
                        )
    ...
    @contextmanager
    def ready_empty_condition(self):
        self._flag_is_getting_empty_condition = True
        try:
            yield
        finally:
            self._flag_is_getting_empty_condition = False
    ...
    def get_empty_conditions(
        ...
        with self.ready_empty_condition():
            conditions = super().get_empty_conditions(bs, negative_text_features=negative_text_features)
            batch_size = conditions.text_f.shape[0]
            conditions.control = { \
                name: torch.zeros((batch_size, self._latent_seq_len, self.empty_control_dim[name])).float().to(self.device) \
                for name in self.control_types \
            }
        return conditions
...
def build_text2audio_model(name: str, **kwargs) -> MeanAudio:
    ...
    if name == 'fluxaudio_fm': 
        if kwargs.get('control_types', None) is not None:
            return control_fluxaudio_fm(**kwargs)
        return fluxaudio_fm(**kwargs)
    if name == 'fluxaudio_m_full': 
        if kwargs.get('control_types', None) is not None:
            return control_fluxaudio_m_full(**kwargs)
