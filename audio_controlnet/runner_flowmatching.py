...
from audio_controlnet.utils.time_estimator import PartialTimeEstimator, TimeEstimator
import wandb
...
class RunnerFlowMatching:
    ...
    def __init__(self,
        ...
        self.fm = FlowMatching(cfg.sampling.min_sigma,
                               ...
                               from_data_to_noise=cfg.get('from_data_to_noise', False)
                               )
    ...
    def train_fn( 
        ...
        **kwargs,
        ...
        # sample
        # breakpoint()
        a_randn = torch.empty_like(a_mean).normal_(generator=self.rng) # a_mean, a_randn: [24, 430, 40]
        x1 = a_mean + a_std * a_randn
        bs = x1.shape[0]  # batch_size * seq_len * num_channels
        
        # copy the randn noise to input so that input VAE latents match the targets
        if 'control' in kwargs and 'inpaint' in kwargs['control']:
            for i in range(len(kwargs['control']['inpaint'])):
                kwargs['control']['inpaint'][i]['a_randn'] = a_randn[i].clone().detach() # [430, 40]
        
        if 'control' in kwargs and 'reference' in kwargs['control']:
            for i in range(len(kwargs['control']['reference'])):
                kwargs['control']['reference'][i]['a_randn'] = a_randn[i].clone().detach() # [430, 40]
        ...
        if self.cfg.get('mask_unmask_ratio', None): # 对mask和非mask部分用不同的loss比例
            ...
            if 'inpaint' in kwargs['control']:
                ...
                target_rate = self.cfg['conditioners_config']['inpaint']['target_rate']
            elif 'remove' in kwargs['control'] or 'insert' in kwargs['control']:
                segments = []
                cond_name = 'remove' if 'remove' in kwargs['control'] else 'insert'
                assert cond_name in kwargs['control']
                all_events = kwargs['control'][cond_name]
                if 'events' in all_events[0]:
                    all_events = [x['events'] for x in all_events]
                for events in all_events:
                    segment = []
                    for v in events.values():
                        segment += v
                    segments.append(segment)
                target_rate = self.cfg['conditioners_config'][cond_name]['target_rate']
            else:
                raise ValueError
            loss_coef = extract_mask_unmask_loss_coef(segments, x1, target_rate, mask_ratio, unmask_ratio)
            loss = self.fm.loss(pred_v, x0, x1, loss_coef=loss_coef)
        else:
    ...
    def train_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            control_kwargs = extract_control_kwargs_from_data(data, self.cfg)
            # breakpoint()
            if 'a_mean' in data and 'target' not in data:
                a_mean = data['a_mean'].cuda(non_blocking=True)
                a_std = data['a_std'].cuda(non_blocking=True)
            else:
                assert 'target' in data
                target_wavs: torch.Tensor = data['target']['audio']
                if target_wavs.ndim == 3:
                    target_wavs = target_wavs.squeeze(dim=1)
                conditioner = self.network.module.conditioners.insert
                a_mean, a_std = conditioner.get_vae_mean_and_std(target_wavs.to(self.rng.device))

            self.log.data_timer.end()
            if it % self.log_extra_interval == 0:
                unmasked_text_f = text_f.clone()
                unmasked_text_f_c = text_f_c.clone()
            x1, loss, mean_loss, t = self.train_fn(text_f, text_f_c, a_mean, a_std, **control_kwargs)
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp,
            ...
            try:
                if it % self.log_extra_interval == 0:
                    ...
                    if len(control_kwargs) > 0:
                        control_kwargs = {k:{kk:vv[0:1] for kk,vv in v.items()} for k,v in control_kwargs.items()}
                    conditions = self.network.module.preprocess_conditions(text_f, text_f_c, **control_kwargs)
    ...
    def validation_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            self.log.data_timer.end()
            loss, mean_loss, t = self.val_fn(text_f.clone(), text_f_c.clone(), x1, **control_kwargs)
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            if 'a_mean' in data and 'target' not in data:
                ...
                if target_wavs.ndim == 3:
                    target_wavs = target_wavs.squeeze(dim=1)
                conditioner = self.network.module.conditioners.insert
                a_mean, _ = conditioner.get_vae_mean_and_std(target_wavs.to(self.rng.device))
